...
const botoesFechar = () => {
    // BOTOES FECHAR MODAL
    selecionaTodos('.produtosInfo--cancelButton, .produtosInfo--cancelMobileButton').forEach( (item) => item.addEventListener('click', fecharModal) )
}
...
const preencheDadosDosprodutos = (produtosItem, item, index) => {
    ...
    produtosItem.querySelector('.produtos-item--price').innerHTML = formatoReal(item.price[2])
    produtosItem.querySelector('.produtos-item--name').innerHTML = item.name
    produtosItem.querySelector('.produtos-item--desc').innerHTML = item.description
}
...
const escolherTamanhoPreco = (key) => {
    ...
    selecionaTodos('.produtosInfo--size').forEach((size, sizeIndex) => {
        size.addEventListener('click', (e) => {
            ...
            seleciona('.produtosInfo--actualPrice').innerHTML = formatoReal(produtosJson[key].price[sizeIndex])
        })
    })
}
...
const adicionarNoCarrinho = () => {
    seleciona('.produtosInfo--addButton').addEventListener('click', () => {
    	...
    	// tamanho
	    let size = seleciona('.produtosInfo--size.selected').getAttribute('data-key')
	    //console.log("Tamanho " + size)
	    // quantidade
    	//console.log("Quant. " + quantprodutoss)
        // preco
        let price = seleciona('.produtosInfo--actualPrice').innerHTML.replace('R$&nbsp;', '')
...
produtosJson.map((item, index ) => {
    ////console.log(item)
    let produtosItem = document.querySelector('.models .produtos-item').cloneNode(true)
    ////console.log(produtosItem)
    ...
    // produtos clicada
    produtosItem.querySelector('.produtos-item a').addEventListener('click', (e) => {
        e.preventDefault()
